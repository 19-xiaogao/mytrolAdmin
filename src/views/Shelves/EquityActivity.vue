<template>
    <div ref="orderDetailRef" class="operation-activity">
        ...
        <div class="add-box">
            <UploadEquityImg ref="cover" uploadName="权益卡封面" @uploadDone="handleEquityCoverDone" />
            <UploadEquityImg ref="content" uploadName="权益卡内容" @uploadDone="handleEquityContentDone" />
            <UploadEquityImg ref="code" uploadName="权益卡二维码" @uploadDone="handleEquityQrCodeDone" />
        </div>
    ...
    setup(props, { emit }) {
        ...
        const uploadFileAllOss = async () => {
            ...
            if (formData.get("equity_cover")) {
                const fileName = `item/equity_cover${uuidToCreateHash()}.${equityParams.equity_cover.type}`;
                const result = await uploadAliOssApi(fileName, formData.get("equity_cover"));
                callBackObj.equity_cover = result.res.requestUrls[0];
            }
            if (formData.get("equity_content")) {
                const fileName = `item/equity_cover${uuidToCreateHash()}.${equityParams.equity_content.type}`;
                const result = await uploadAliOssApi(fileName, formData.get("equity_content"));
                callBackObj.equity_content = result.res.requestUrls[0];
            }
            if (formData.get("qr_code")) {
                const fileName = `item/qr_code${uuidToCreateHash()}.${equityParams.qr_code.type}`;
